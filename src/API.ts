...
export default class API {
  ...
  static open(devicePath: string, 
    {baudRate, parity = 0, dataBits = 8, stopBits = 1, chunkSize = 0, timeoutMs = -1, sleepMs = -1, delimiter = -1}: { baudRate: number, parity?: number, dataBits?: number, stopBits?: number, chunkSize?: number, timeoutMs?: number, sleepMs?: number, delimiter?: number }) {
    if (Platform.OS !== 'android') throw new Error(`Not support ${Platform.OS}`)
